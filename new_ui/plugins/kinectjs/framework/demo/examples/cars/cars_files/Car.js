...
THREE.Car = function () {
	...
	this.MAX_SPEED = 2600;
	this.MAX_REVERSE_SPEED = -1600;

	this.MAX_WHEEL_ROTATION = 0.6;

	this.FRONT_ACCELERATION = 1300;
	...
	this.updateCarModel = function ( delta, controls ) {
		...
		if ( controls.moveForward ) {
			this.speed = clamp( this.speed + delta * this.FRONT_ACCELERATION, this.MAX_REVERSE_SPEED, ( -( this.MAX_SPEED * ( kinect.coords[ 0 ][ 0 ].z / 90 ) ) >> 0 ) );
		...
		//if ( controls.moveLeft ) {
			this.wheelOrientation = controls.wheelOrientation;
			//console.log(this.wheelOrientation );
